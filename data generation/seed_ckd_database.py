...
async def seed_ckd_database():
    ...
    try:
        async with async_session() as session:
            ...
            if "doctors" in data:
                print("\n⚠️  Skipping CKD doctors seeding - doctors should be created as patients first")
            ...
            if "labs" in data:
                ...
                print(f"✅ Seeded {len(data['labs'])} CKD labs")
            
            # 3. Seed Patients
            if "patients" in data:
                print("\n👥 Seeding CKD patients...")
                ...
                print(f"✅ Seeded {len(data['patients'])} CKD patients")
            
            # 4. Seed Family Relationships
            if "family_relationships" in data:
                print("\n👨‍👩‍👧‍👦 Seeding CKD family relationships...")
                ...
                await session.commit()
                print(f"✅ Seeded {len(data['family_relationships'])} CKD family relationships")
            
            # 5. Seed Family Disease History
            if "family_disease_history" in data:
                print("\n🧬 Seeding CKD family disease history...")
                for disease_data in data["family_disease_history"]:
                    ...
                    session.add(disease)
                await session.commit()
                print(f"✅ Seeded {len(data['family_disease_history'])} CKD family disease records")
            
            # 6. Seed Doctor Visits
            if "doctor_visits" in data:
                print("\n📋 Seeding CKD doctor visits...")
                batch_size = 1000
                ...
                print(f"✅ Seeded {len(data['doctor_visits'])} CKD doctor visits")
            
            # 7. Seed Symptoms
            if "symptoms" in data:
                print("\n🤒 Seeding CKD symptoms...")
                batch_size = 1000
                ...
                print(f"✅ Seeded {len(data['symptoms'])} CKD symptoms")
            
            # 8. Seed Diagnoses
            if "diagnoses" in data:
                print("\n🩺 Seeding CKD diagnoses...")
                batch_size = 1000
                ...
                print(f"✅ Seeded {len(data['diagnoses'])} CKD diagnoses")
            
            # 9. Seed Prescriptions
            if "prescriptions" in data:
                print("\n💊 Seeding CKD prescriptions...")
                batch_size = 1000
                ...
                print(f"✅ Seeded {len(data['prescriptions'])} CKD prescriptions")
            
            # 10. Seed Lab Reports
            if "lab_reports" in data:
                print("\n📊 Seeding CKD lab reports...")
                batch_size = 1000
                ...
                print(f"✅ Seeded {len(data['lab_reports'])} CKD lab reports")
            
            # 11. Seed Lab Test Results
            if "lab_test_results" in data:
                print("\n🧪 Seeding CKD lab test results...")
                batch_size = 5000
                ...
                print(f"✅ Seeded {len(data['lab_test_results'])} CKD lab test results")
            
            # 12. Seed Disease Progressions
            if "disease_progressions" in data:
                print("\n📈 Seeding CKD disease progressions...")
                ...
                await session.commit()
                print(f"✅ Seeded {len(data['disease_progressions'])} CKD disease progressions")
            
            print("\n" + "="*60)
            print("✅ CKD DATABASE SEEDING COMPLETED SUCCESSFULLY!")
            print("="*60)
